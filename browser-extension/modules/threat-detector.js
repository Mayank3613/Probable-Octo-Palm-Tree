...
(function() {
  ...
  const ThreatDetector = {
    ...
    scanFakeLoginForms: function() {
      ...
      // Check if form submits to a different domain (credential exfiltration)
      for (let form of forms) {
        ...
        try {
          ...
          if (actionUrl.hostname !== window.location.hostname) {
            // Form submits credentials to a different domain
      ...
      // Check for suspicious form characteristics (no real brand but asking for credentials)
      if (passwordInputs.length > 0 && forms.length > 0) {
        ...
        if (emailInputs.length > 0) {
          // Likely a login page — check if domain is suspicious
